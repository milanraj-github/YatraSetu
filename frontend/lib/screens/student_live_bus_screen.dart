...
class StudentLiveBusScreen extends StatefulWidget {
  const StudentLiveBusScreen({super.key});

  @override
  State<StudentLiveBusScreen> createState() => _StudentLiveBusScreenState();
}

class _StudentLiveBusScreenState extends State<StudentLiveBusScreen> {
  Timer? _timer;
  ...
  Future<void> _fetchLiveBus() async {
    try {
      final res = await ApiService.getStudentLiveBus();
      if (!mounted) return;
  ...
  String _getEtaText() {
    ...
    if (status == 'AVAILABLE') {
      ...
      return 'Bus is not currently on a trip';
  ...
  Widget _buildStatusBanner() {
    ...
    switch (_trackingStatus) {
      ...
      case "NO_ACTIVE_TRIP":
        ...
        statusText = "Bus is not currently on a trip";
        break;
      default:
        bgColor = Colors.grey;
