...
import 'parent_safety_screen.dart';
import 'parent_trip_history_screen.dart';
...
class _ParentPortalScreenState extends State<ParentPortalScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final unreadRes = await ApiService.getUnreadNotificationCount();
      if (unreadRes['success'] == true && unreadRes['data'] != null) {
        _unreadCount = unreadRes['data']['unread_count'] ?? 0;
      }
      ...
      if (res['success'] == true) {
        ...
        setState(() {
          _errorMessage =
              res['error']?.toString() ?? 'Failed to load students.';
          _isLoading = false;
        });
      }
    } catch (e) {
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (_) => ParentTripHistoryScreen(
                            studentId: student['id'],
                            studentName: student['full_name'],
                          ),
                        ),
  ...
  Widget build(BuildContext context) {
    final List<Widget> pages = [
                      ...
                      child: Row(
                        children: List.generate(_students.length, (index) {
                          ...
                          return Padding(
                            ...
                            child: ChoiceChip(
                              label: Text(
                                _students[index]['student']['full_name'],
                              ),
                              selected: isSelected,
                              onSelected: (selected) {
                                if (selected) {
      ...
      const ParentSafetyScreen(),
      const ParentNotificationsScreen(),
      ParentProfileScreen(initialProfile: _profile, students: _students),
    ];
      ...
      bottomNavigationBar: Theme(
        ...
        child: BottomNavigationBar(
          ...
          onTap: (index) {
            ...
            if (index == 2) {
              _fetchData(); // Refresh notifications
            }
          },
          items: [
